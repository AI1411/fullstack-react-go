...
func newOperationLog(db *gorm.DB, opts ...gen.DOOption) operationLog {
	...
	_operationLog.ResourceType = field.NewString(tableName, "resource_type")
	_operationLog.ResourceID = field.NewString(tableName, "resource_id")
	_operationLog.Endpoint = field.NewString(tableName, "endpoint")
	...
	_operationLog.RequestData = field.NewString(tableName, "request_data")
	_operationLog.ResponseStatus = field.NewInt32(tableName, "response_status")
	_operationLog.ExecutionTimeMs = field.NewInt32(tableName, "execution_time_ms")
...
type operationLog struct {
	...
	ALL             field.Asterisk
	ID              field.Int64  // 監査ログID（主キー、自動掲番）
	UserID          field.String // ユーザーID（外部キー、ユーザーテーブルのID）
	Action          field.String // 実行されたアクション（例: create, update, delete）
	ResourceType    field.String // リソースの種類（例: user, organization, role）
	ResourceID      field.String // リソースID（アクションが適用されたリソースのID）
	Endpoint        field.String // APIエンドポイント（例: /api/users, /api/organizations）
	Method          field.String // HTTPメソッド（例: GET, POST, PUT, DELETE）
	IPAddress       field.String // ユーザーのIPアドレス
	UserAgent       field.String // ユーザーエージェント情報（ブラウザやデバイス情報）
	RequestData     field.String // リクエストデータ（JSON形式）
	ResponseStatus  field.Int32  // レスポンスステータスコード（例: 200, 404, 500）
	ExecutionTimeMs field.Int32  // リクエストの実行時間（ミリ秒単位）
	CreatedAt       field.Time   // 監査ログの作成日時
...
func (o *operationLog) updateTableName(table string) *operationLog {
	...
	o.RequestData = field.NewString(table, "request_data")
	o.ResponseStatus = field.NewInt32(table, "response_status")
	o.ExecutionTimeMs = field.NewInt32(table, "execution_time_ms")
...
func (o *operationLog) fillFieldMap() {
	...
	o.fieldMap["request_data"] = o.RequestData
	o.fieldMap["response_status"] = o.ResponseStatus
	o.fieldMap["execution_time_ms"] = o.ExecutionTimeMs
