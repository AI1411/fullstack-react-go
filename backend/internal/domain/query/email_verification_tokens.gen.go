...
func newEmailVerificationToken(db *gorm.DB, opts ...gen.DOOption) emailVerificationToken {
	...
	_emailVerificationToken.Token = field.NewString(tableName, "token")
	_emailVerificationToken.Email = field.NewString(tableName, "email")
	...
	_emailVerificationToken.IsUsed = field.NewBool(tableName, "is_used")
...
type emailVerificationToken struct {
	...
	ALL       field.Asterisk
	ID        field.String // トークンの一意な識別子
	UserID    field.String // 関連するユーザーのID
	Token     field.String // メール認証トークン
	Email     field.String // 認証対象のメールアドレス
	ExpiresAt field.Time   // トークンの有効期限
	UsedAt    field.Time   // トークンが使用された日時
	IsUsed    field.Bool   // トークンが使用されたかどうか
	CreatedAt field.Time   // トークンの作成日時
...
func (e *emailVerificationToken) updateTableName(table string) *emailVerificationToken {
	...
	e.IsUsed = field.NewBool(table, "is_used")
